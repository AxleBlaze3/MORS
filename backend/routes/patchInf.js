...
router.patch('/patc',async (req,res)=>{
    let cities
    let cityId
    let cityLat
    let cityLng
    ...
    for(var o=0;o<cities.length;o++){
        ...
        let aqiI
        try{
            polut=await pollCollection.findOne({cityId:cityId})
            aqiI=await aqiInfo.findOne({cityId:cityId})
    ...
    pollutants=["PM2.5","PM10","NO2","NH3","SO2","CO"]
      ...
      if(i==response.data.records.length-1){
       ...
       //console.log(pollutantList)
        ...
        aqiI.aqi=result.aqi,
        aqiI.healthEffects=result.healthEffects,
        aqiI.guidanceStatement=result.guidanceStatement,
        aqiI.aqiVal=result.aqiVal,
        aqiI.pollutant_id=result.pollutant_id,
        aqiI.category=result.category
